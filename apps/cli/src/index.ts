...
import { copyExtensionBuild } from "./extension";
import { nativeHostName } from "./native/constants";
	...
	try {
		...
		switch (command.kind) {
			...
			case "extension-build": {
				...
				process.stdout.write(`Extension copied to: ${result.outPath}\n`);
				return 0;
			}
...
Usage:
  atlas-browser-connect extension build [--out <path>]
