...
function createProgram(setCommand: (command: CliCommand) => void) {
	...
	extension
		...
		.option(
			...
			"Comma-separated Chrome permissions (e.g. tabs,scripting,debugger). Defaults to tabs,tabGroups,activeTab,bookmarks,windows.",
		)
		.action((options: { out?: string; permissions?: string }) => {
			setCommand({
